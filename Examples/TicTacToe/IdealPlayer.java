	...
	private int tryWinOrBlock(char whichPlayer) {
		...
		for (int pass = 1, step = randomizeMoves ? rand.nextInt(2) : 0;
			...
			if (step == 0) {
				...
				if ( (matchCount == 2) && (spaceCount == 1) ) {
					return isFree(spaceIdx, spaceIdx);
				}
			} else {
				for (rci = 0; rci < 3; rci++) {
					rc = rcs[rci];
				...
				if ( (matchCount == 2) && (spaceCount == 1) ) {
					return isFree(spaceIdx, 2-spaceIdx);
				}
			}
		}
		return -1;
...
/// TODO: If randomizing moves, randomize row/column order. Be careful not to mess up the column delta (cd).
				...
				if ( (myCount > 0) && (opponentCount == 0) && (openr >= 0) && (openc >= 0) ) {
					...
					for (forkIdx = 0; forkIdx < forkCount; forkIdx++) {
						if (   (freeRow[forkFreeIdx[forkIdx]] == openr)
							&& (freeColumn[forkFreeIdx[forkIdx]] == openc)   ) {
							found = true;
							break;
						}
					}
	...
	private int tryOppositeCornerFromOpponent(char whichPlayer) {
		...
		for (int si = 0; si < 4; si++) {
			switch (steps[si]) {
			case 0:
				if ( (grid[0][0] == opponent) && (grid[2][2] == ' ') ) return isFree(2, 2);
				break;
			case 1:
				if ( (grid[0][2] == opponent) && (grid[2][0] == ' ') ) return isFree(2, 0);
				break;
			case 2:
				if ( (grid[2][2] == opponent) && (grid[0][0] == ' ') ) return isFree(0, 0);
				break;
			case 3:
				if ( (grid[2][0] == opponent) && (grid[0][2] == ' ') ) return isFree(0, 2);
				break;
			}
		}
		return -1;
	...
	private int tryAnyCorner(char whichPlayer) {
		...
		for (int si = 0; si < 4; si++) {
			switch (steps[si]) {
			case 0:
				if (grid[0][0] == ' ') return isFree(0, 0);
				break;
			case 1:
				if (grid[0][2] == ' ') return isFree(0, 2);
				break;
			case 2:
				if (grid[2][0] == ' ') return isFree(2, 0);
				break;
			case 3:
				if (grid[2][2] == ' ') return isFree(2, 2);
				break;
			}
		}
		return -1;
	...
	private int tryAnySide(char whichPlayer) {
		...
		for (int si = 0; si < 4; si++) {
			switch (steps[si]) {
			case 0:
				if (grid[0][1] == ' ') return isFree(0, 1);
				break;
			case 1:
				if (grid[1][2] == ' ') return isFree(1, 2);
				break;
			case 2:
				if (grid[2][1] == ' ') return isFree(2, 1);
				break;
			case 3:
				if (grid[1][0] == ' ') return isFree(1, 0);
				break;
			}
		}
		return -1;
